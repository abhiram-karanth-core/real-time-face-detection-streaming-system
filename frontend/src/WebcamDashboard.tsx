...
export default function WebcamDashboard() {
  ...
  const outWsRef    = useRef<WebSocket | null>(null);
  ...
  const connect = useCallback(() => {
    ...
    const wsInUrl = `${WS_URL.replace("/ws/stream", "/ws/stream/in")}?session_id=${sessionRef.current}`;
    const wsIn = new WebSocket(wsInUrl);
    wsRef.current = wsIn;
    ...
    wsIn.onerror = () => {
      setStatus("error");
      setError("Input WebSocket connection failed.");
    };
    ...
    const wsOutUrl = `${WS_URL.replace("/ws/stream", "/ws/stream/out")}?session_id=${sessionRef.current}`;
    const wsOut = new WebSocket(wsOutUrl);
    outWsRef.current = wsOut;

    wsOut.onmessage = (event: MessageEvent) => {
    ...
    wsOut.onerror = () => {
      console.error("Output WebSocket failed.");
    };
  }, []);
